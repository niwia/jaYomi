...
// Helper to generate DMM CDN image URLs instantly
...
// Verify if DMM URL exists, because if it doesn't, it 302 redirects to a "now printing" placeholder.
async function verifyDmmPoster(url) {
    if (!url) return false;
    try {
...
async function fetchMetadata(query) {
    ...
    try {
        ...
        // If JavDatabase fails, quickly verify and drop-in the DMM URL
        const dmmUrl = getDmmPosterUrl(id);
        const isValid = await verifyDmmPoster(dmmUrl);
        if (isValid) {
            metadata.poster = dmmUrl;
        } else {
            console.log(`[Metadata] DMM fallback also missed ${id} (returned 302 now_printing). Returning blank metadata.`);
        }
    }
    
    // We cache it so we don't spam requests
    ...
    // If it has NO poster and NO title, we return null so the addon.js falls back to dummy poster generator
    if (!metadata.poster && !metadata.title) return null;
